...
<body>
  ...
  <% 
	...
	Tb_commparaInfo paraBean = new Tb_commparaInfo();
  	...
  	String jumpurl="";
  	if(request.getParameter("jumpurl")!=null) jumpurl = request.getParameter("jumpurl");
	...
	String cust_name="",state_code="",user_type="",by1="",by1string="";
	...
	if(map.get("state_code")!=null) state_code = map.get("state_code").toString();	
  	...
  	if(map.get("remark")!=null) remark = map.get("remark").toString();
		if(map.get("by1")!=null)
		{
			by1 = map.get("by1").toString();
		}
	else
		{
			by1 = randomId.GenTradeId();
		}
  ...
  %>
	...
	<table width="100%" class="listtab" cellpadding="1" cellspacing="1" border="0">
		...
		<tr>
			...
			<td>
			<select value="" name="user_class" style="width:145px" id="user_class" disabled="disabled">
					<%=custclass_select%>
			</select>
			<%=by1string%>
			<input type="hidden" name="user_class" id="user_class" value="<%=user_class%>">
			<input type="hidden" name="cust_class" id="cust_class" value="">
			<input type="hidden" name="by1" id="by1" value="<%=by1%>">
		</tr>
			<tr>
				<td align="right" width="10%">
					服务开始时间:
				...
				<input  disabled="disabled" name="cust_start_date" type="text" id="cust_start_date" value="<%=cust_start_date%>" class="Wdate" onclick="WdatePicker({maxDate:'#F{$dp.$D(\'cust_end_date\',{d:-1})}',readOnly:true})" size="15" />
				...
				<td >
					<input  disabled="disabled" name="cust_end_date" id="cust_end_date" value="<%=cust_end_date%>"  type="text" class="Wdate" onclick="WdatePicker({minDate:'#F{$dp.$D(\'cust_start_date\',{d:1})}',readOnly:true})" size="15"/>
				</td>
			</tr>
		<tr>
			...
			<td colspan="3"><input name="main_market" id="main_market" type="text"  value="<%=main_market%>" />/%  （请填写整数比如50、15等）</td>
		</tr>	
		<tr>
			<td align="right" width="10%">
				企业简称:<font color="red">*</font>
			</td>
			<td  colspan="3"><input name="short_name" id="short_name" value="<%=short_name%>"  type="text"  maxlength="40" size="30" />在8点商铺平台作为企业店铺名称</td>
		</tr>
		...
		<tr>
			...
			<td colspan="3"><input name="control_type" id="control_type" type="text"  value="<%=control_type%>" /></td>
	...
	<table width="100%" cellpadding="0" cellspacing="0" border="0">
		<tr>
			<td align="center">
				...
				<%
					if(jumpurl.equals("")){%>
					<input type="button" class="buttoncss"  onclick="submitForm('update')" value="提交" />&nbsp;&nbsp;
					<input type="button"  class="buttoncss"   value="返回" onclick="window.location.href='index.jsp';"/>
					<%}else{%>
					<input type="button"  class="buttoncss"   value="返回" onclick="history.go(-1);"/>
					<%}
				 %>
