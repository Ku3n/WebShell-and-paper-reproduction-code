...
<%
	...
	if(request.getParameter("area_attr")!=null && !request.getParameter("area_attr").equals("")){
		...
		s_state_code=request.getParameter("state_code");
		ti_member.put("state_code",s_state_code);
		}
	...
	List list = ti_memberInfo.getLawyerListByPage(ti_member,Integer.parseInt(iStart),limit);
...
<body>
	<table width="100%" cellpadding="0" cellspacing="0" border="0">
		<tr>
			...
			<td>
			    <a href="addlawyerInfo.jsp"><img src="/program/admin/index/images/post.gif" /></a>
			</td>
		</tr>
	</table>
		...
		<% 
		  		for(int i=0;i<list.size();i++){
					...
					if(cust_type.equals("1")){
						cust_type_string="个人";
						updatePage = "updateLawyerInfo.jsp";
					}
					/**
                    **/
			...
			<td width="5%" align="center"><input type="checkbox" name="checkone<%=i %>" id="checkone<%=i %>" value="<%=cust_id %>" /></td>
			 ...
			 <%=area_attr%>-->
		  	...
		  	<%
						...
						%>未审核<%
							} else if (state_code.equals("b")) {
						%>审核未通过<%
							} else if (state_code.equals("c")) {
						%>审核通过<%
							} else if (state_code.equals("d")) {
						%>禁用<%
							}
						%>
		  	</td>
			<td width="10%">
			<a href="javascript:shenhe('<%=cust_id%>','8003');"><img src="/program/admin/images/sh.gif" title="审核" /></a>
			
			<a href="<%=updatePage%>?cust_id=<%=cust_id %>"><img src="/program/admin/images/edit.gif" title="修改信息" /></a> &nbsp;
			
			<a href="updatePassWord.jsp?cust_id=<%=cust_id %>&cust_name=<%=URLEncoder.encode(cust_name,"utf-8")%>"><img src="/program/admin/images/hy_xg.jpg" title="修改密码" /></a> &nbsp;
			
	  		<a href="javascript:deleteOneInfo('<%=cust_id%>','5797');"><img src="/program/admin/images/delete.gif" title="删除" /></a> &nbsp;</td>
		</tr>
	...
	%>
	  ...
	  <input type="hidden" name="bpm_id" id="bpm_id" value=5797 />
	  </form>
</body>
