...
class WebshellDetector:
    ...
    def fit(self, features_list, labels, n_splits=5):
        """使用交叉验证训练并评估模型"""
        logger.info(f"开始使用{self.model_type}进行 {n_splits} 折交叉验证")
        ...
        for fold, (train_index, test_index) in enumerate(skf.split(X, y), 1):
            ...
            logger.info(f"训练集: {Counter(y_train)}, 测试集: {Counter(y_test)}")
        ...
        logger.info(f"F1分数: {avg_metrics['f1']:.4f}")
...
def main():
    # 设置数据路径
    benign_dir = 'D:/DeepLearning/otherpaper/two/jsp/normal'  # 良性文件目录
    malicious_dir = 'D:/DeepLearning/otherpaper/two/jsp/shell'  # 恶意文件目录
