...
<%
	...
	if(sublist!=null&&sublist.size()>0){
		...
		if(submap.get("main_market")!=null && !submap.get("main_market").toString().equals(""))
	...
	String pageString = new PageTools().getGoogleToolsBar(counter,"bizlist.jsp?iStart=",Integer.parseInt(iStart),limit);
  /**分公司提成计算方式：(商品销售总价格 X 分公司协议提成比例)**/
  List totalList=bizInfo.getTotalPriceByAreaAttr(bizmap);
	...
	if(totalList!=null&&totalList.size()>0){
		...
		if(bizmap.get("income_num")!=null && !bizmap.get("agent_product_code").toString().equals(""))
			all_income_num=String.valueOf(Math.rint((Double.parseDouble(mainmarket)/100)*Double.parseDouble(all_agent_product_code)));
	}
...
<body>
	...
	<table width="100%" cellpadding="0" cellspacing="0" border="0" class="dl_bg">
		<tr>
			<td width="90%" style="font-size:16px">
				 ...
				 <%=cust_name %>提成比例为<%=mainmarket%>% 所获提成合计为：<%=all_income_num%>&nbsp;&nbsp;
		...
		<% 
		  		for(int i=0;i<list.size();i++){
		  			...
		  			is_settle="",settle_date="";
						...
						if (!is_settle.equals("1")) income_num = "0";
	...
	%>
	  ...
	  <input type="hidden" name="bpm_id" id="bpm_id" value="5357" />
	  <input type="hidden" name="cust_id" id="cust_id" value="<%=cust_id %>" />
	  <input type="hidden" name="pkid" id="pkid" value="<%=pkid %>" />
	  <input type="hidden" name="is_pay" id="is_pay" value="1" />
	  <input type="hidden" name="income_ratio" id="income_ratio" value="<%=mainmarket%>" />
	  ...
	  <input type="hidden" name="agent_product_codee" id="agent_product_codee" value="<%=all_agent_product_code %>" />
	 
	  <input type="hidden" name="num" id="num" value="<%=(int)new_price %>" />
	  <input type="hidden" name="type" id="type" value="1" />
	  <input type="hidden" name="jiesuan" id="jiesuan" value="1" />
	   <input type="hidden" name="reason" id="reason" value="分公司业绩结算" />
	  </form>
