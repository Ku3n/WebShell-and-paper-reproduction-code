...
<body>
	<table width="100%" cellpadding="0" cellspacing="0" border="0">
		<tr>
			...
			<!--td>
			    <a href="addCompanyInfo.jsp"><img src="/program/admin/index/images/post.gif" /></a>
			</td-->
		</tr>
	</table>
	...
	<table width="100%" cellpadding="1" cellspacing="1" class="listtab" border="0">
		...
		<tr>
			<td width="5%" align="center"><input type="checkbox" name="checkone<%=i %>" id="checkone<%=i %>" value="<%=cust_id %>" /></td>
		  	...
		  	<td><a href="/data/company/<%=cust_id%>/index.html" target="_blank"><%=cust_name%></a></td>
			...
			<td width="50%">
					<%
						if (!file.exists()){ 		
							if(req_url.indexOf("8diansp.com")!=-1)
							{
						%>
		  	    <p><input type="button" name="delInfo" onClick="tijiao('<%=cust_id %>','/templets/company/templets/c2c/')" value="开通网店" class="buttab" /></p>         
		  	    <%
		  	    	}else
		  	    	{
		  	    %>
		  	    <p><input type="button" name="delInfo" onClick="tijiao('<%=cust_id %>','/templets/company/templets/default/')" value="开通网站" class="buttab" />  
	 					<%
	 						}
 						}
 						else
 						{
 					%> 					 
 					 <p>	<input type="button" name="delInfo" onClick="deletejs('<%=cust_id %>')" value="关闭网站" class="buttab" />  
 						<%
 						if(req_url.indexOf("8diansp.com")!=-1)
							{
						%>
		  	    <input type="button" name="delInfo" onClick="tijiao('<%=cust_id %>','/templets/company/templets/c2c/')" value="重新开通" class="buttab" />   </p>       
		  	    <%
		  	    	}else
		  	    	{
		  	    %>
		  	    <input type="button" name="delInfo" onClick="tijiao('<%=cust_id %>','/templets/company/templets/default/')" value="重新开通" class="buttab" />   </p>
 					...
 					%>
	  		  &nbsp;
	  			<div id="backMessage<%=cust_id %>"></div>
	  		</td>
