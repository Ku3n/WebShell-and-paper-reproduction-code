		...
		<tr>
			...
			<td width="10%"><img src="<%=img_path%>" border="0" width="85" height="85" /></td>
		  	...
		  	<td width="30%">
	            ...
	            <span class="<%if(state_code.indexOf("d")>-1)out.print("blueon"); else out.print("blueoff");%>">关闭</span> 
			</td>
		  	...
		  	<td width="10%">
		  		<%
		  			...
		  			else 
		  		 {
		  		 		...
		  		 		{
		  		 			ordersum = "[ <font style=color:blue>未达到最低门限</font> ]<br>合计订购数量："+ordersum;
		  		 		}
		  		 }
		  		%>
		  		<%=ordersum%>
			...
			<a href="orderlist.jsp?biz_id=<%=biz_id%>&s_title=<%=s_title%>&state_c=<%=state_c%>&info_state=<%=info_state%>&type=<%=type%>&iStart=<%=Integer.parseInt(iStart)%>">订单列表</a>
