...
<script type="text/javascript">
		function sub(){
			if(document.frm.userpwd.value.length<=5){
				alert("密码长度少于六位数!");
				return;
			}
...
<table class="tableborder" cellspacing="1" cellpadding="3" align="center" border="0">
	...
	<form action="<%=url%>/regservlets" method="post" name="frm">
		...
		<tr>
			...
			<td class="tablerow1" width="70%"><input name="username" type="text" value="<%=ub.getUserName()%>" size="30" maxlength="30" readonly/></td>
		</tr>
		<input type="hidden" name="action" value="update">
		<tr>
			<td class="tablerow2" align="right"><strong>用户密码：</strong></td>
			<td class="tablerow2"><input type="text" name="userpwd" value="<%=ub.getUserPwd()%>" size="30" maxlength="30"/></td>
		</tr>
		<tr>
			<td class="tablerow1" align="right"><strong>用户邮箱：</strong></td>
			<td class="tablerow1"><input size="30" name="email" value="<%=ub.getUserEmail()%>" maxlength="30" type="text" /></td>
		</tr>
		<tr>
			<td class="tablerow2" align="right"><strong>密保问题：</strong></td>
			<td class="tablerow2"><input name="question" type="text" value="<%=ub.getQuestion()%>" readonly/></td>
		</tr>
		<tr>
			<td class="tablerow2" align="right"><strong>密保答案：</strong></td>
			<td class="tablerow2"><input name="result" type="text" value="*********" readonly/></td>
		</tr>
		<tr>
			...
			<option value="女" selected="selected">美女</option>
			<%}%>
			</select></td>
		</tr>
		...
		<tr>
			...
			<td class="tablerow2"><input size="20" name="weburl" value="<%=ub.getWebAddress()%>" type="text"  maxlength="30"/></td>
		</tr>
		<tr>
			<td class="tablerow2" align="right"><strong>QQ:</strong></td>
			<td class="tablerow2"><input size="20" name="qq" value="<%=ub.getQQ()%>" type="text" maxlength="30"/></td>
		</tr>
		<tr>
		...
		<tr align="center">
			<td class="tablerow1" colspan="2"><input class="Button" type="button" onclick="javascript:history.go(-1)" name="Submit2" value="返回上一页" /> <input class="Button" type="button" name="Submit1" value="保存修改" onclick="sub();"/></td>
		</tr>
	</form>
</table>
