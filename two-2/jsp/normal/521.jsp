...
<html>
		...
		if(confirm("你确定要导入吗")){
			$.ajax({
				  ...
				  success : function(returnData){
						if(returnData == 'true'){
							$("#loading").html('<span style="color:red;font-size:16px;">导 入 成 功</span>');
							sleep(3000);
							window.parent.location.href = ctx+"/good!list.do";
						}else{
							$("#loading").html('<span style="color:red;font-size:16px;">导入失败，请重新操作</span>');
						}
				  },
				  error : function(){
						$("#loading").html('<span style="color:red;font-size:16px;">系 统 有 误</span>');
				  } 
			});
		}
	}
  ...
  <body>
    	...
    	<div id="loading" align="center" style="display:none">
    		<span style="color:red;font-size:16px;">数据在导入,请耐心等待</span>
    	</div>
  </body>
</html>
