			...
			<table width="100%" cellpadding="1" cellspacing="1" border="0"
				...
				<tr>
					<td>
						...
						<a
							href="/program/admin/tsmodule/filelist.jsp?file_path=<%=file_path_tem%>&cur_page=<%=cur_page %>&ch_id=<%=ch_id %>">上级目录</a>
						<%
							}
						%>
					</td>
				...
				<td width="10%">
						<%
							if (isdirectory) {
						%>
						<a href="/program/admin/tsmodule/filelist.jsp?file_path=<%=file_path_mo%>&cur_page=<%=cur_page %>&ch_id=<%=ch_id %>">
						<img src="/program/admin/images/<%=file_ending%>.gif" /><%=filelist[i].getName()%> <%
 	} else {
 %>

					<a href="#" onclick="setIndexTemp('<%=cur_page %>','<%="/templets/html"+file_path + '/'+filelist[i].getName()%>','<%=ch_id %>')"><img src="/program/admin/images/<%=file_ending%>.gif" /><%=filelist[i].getName()%> </a>
							<%
 	}
 %>
